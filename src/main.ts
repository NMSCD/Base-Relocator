...
document.getElementById('copyCheckbox')?.addEventListener('change', function () {
  checkboxChange((this as unknown as HTMLInputElement).checked);
});
document.getElementById('reset')?.addEventListener('click', () => reset());
document.getElementById('copy')?.addEventListener('click', function () {
  copyButton(this as unknown as HTMLButtonElement);
});
document.getElementById('swap')?.addEventListener('click', function () {
  swapBases(this as unknown as HTMLButtonElement);
});
document.getElementById('JSONinput')?.addEventListener('input', function () {
  readJSON(this as unknown as HTMLTextAreaElement);
});
...
function readJSON(jsonInput: HTMLTextAreaElement): void {
  ...
  if (!isValid) {
    if (jsonString) {
      warn(
        'JSON is not correctly formatted. Please make sure you copied the whole PersistentPlayerBases section.',
        true
      );
    }
...
function swapBases(button: HTMLButtonElement) {
  ...
  if (
    ...
    !selectedElements[0] ||
    !selectedElements[1] ||
    selectedElements[0]?.dataset?.id === selectedElements[1]?.dataset?.id
  ) {
